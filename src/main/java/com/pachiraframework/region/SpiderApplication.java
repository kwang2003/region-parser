...
public class SpiderApplication {
	private File outputFile = new File("d:/regions/05.txt");
...
//					text = text.substring(0, text.indexOf("<br>"));
	...
	private Document parse(String url) throws InterruptedException {
		try {
			Document document = Jsoup.parse(new URL(url), 5000);
			Elements titles = document.getElementsByTag("title");
			
			if(titles.isEmpty()||Strings.isNullOrEmpty(titles.get(0).text())) {
				System.out.println("****************************");
				return parse(url);
			}
			return document;
...
//		String fileName = "d:/regions/04.txt";
		...
		Files.asCharSink(outputFile,charSet , FileWriteMode.APPEND).write(text.toString()); 
	}
	 ...
	 * @throws InterruptedException 
	 */
	private void parseCities(String href) throws MalformedURLException, IOException, InterruptedException {
		...
		for(int i = 0;i < elements.size();i++) {
			...
			parseRegions(td0.getElementsByTag("a").get(0).attr("href"),region.getCode());
		}
	}
	 ...
	 * @throws InterruptedException 
	 */
	private void parseRegions(String href,String citycode) throws MalformedURLException, IOException, InterruptedException {
		Document document = parse("http://www.stats.gov.cn/tjsj/tjbz/tjyqhdmhcxhfdm/2017/"+href);
		...
		for(int i = 0;i < elements.size();i++) {
			...
			if(as.isEmpty()) {
				...
				Region region = Region.builder().code(td0.text()).parent(citycode).level(3).name(td1.text()).build();
				output(region);
			}else {
				...
				Region region = Region.builder().code(code).parent(citycode).level(3).name(name).build();
				output(region);
				parseTowns(href.substring(0, href.indexOf("/")+1)+link,region.getCode());
			}
	 ...
	 * @throws InterruptedException 
	 */
	private void parseTowns(String link,String regioncode) throws MalformedURLException, IOException, InterruptedException {
		...
		for(int i = 0;i < elements.size();i++) {
			...
			Elements as = td0.getElementsByTag("a");
			String code = as.get(0).text();
			Region region = Region.builder().level(4).code(code).parent(regioncode).name(td1.getElementsByTag("a").get(0).text()).build();
			output(region);
			parseVillages(link.substring(0, link.lastIndexOf("/")+1)+as.get(0).attr("href"),region.getCode());
		}
	}
	 ...
	 * @throws InterruptedException 
	 */
	private void parseVillages(String link,String towncode) throws MalformedURLException, IOException, InterruptedException {
		Document document = parse("http://www.stats.gov.cn/tjsj/tjbz/tjyqhdmhcxhfdm/2017/"+link);
		...
		for(int i = 0;i < elements.size();i++) {
			...
			Region region = Region.builder().level(5).code(td0.text()).parent(towncode).name(td2.text()).build();
			output(region);
		}
	}
}
